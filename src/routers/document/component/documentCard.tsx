...
interface DocumentCardProps {
  ...
  actions?: TableAction<IDocumentIndex>[];
  scope: string[];
}

export const DocumentCard: React.FC<DocumentCardProps> = ({ record, actions = [], scope }) => {
  const getInitials = () => {
    const init = record.documentType || 'd';
    return init.toString().substring(0, 2).toUpperCase();
  };

  const filteredActions = actions.filter((action: TableAction<IDocumentIndex>) => {
      ...
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-foreground text-lg truncate">{record.documentType || 'Unknown Document'}</h3>
              <p className="text-sm text-muted-foreground flex items-center mt-1">
                <Hash className="w-3 h-3 mr-1 flex-shrink-0" />
                <span className="truncate">{record.documentId || 'N/A'}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center text-sm text-foreground bg-gradient-to-r from-primary/5 to-primary/10 p-3 rounded-lg">
            ...
            <span className="truncate font-medium">{record.documentType}</span>
          </div>
        </div>
        ...
        {filteredActions.length > 0 && (
          <div className="mt-4 pt-4 border-t border-border/50">
            <div className="flex gap-2">
              {filteredActions.map((action: TableAction<IDocumentIndex>, index: number) => (
                <Button
