...
interface LearningPlanCardProps {
  ...
  actions?: TableAction<ILearningPlanIndex>[];
  scope: string[];
}

export const LearningPlanCard: React.FC<LearningPlanCardProps> = ({ record, actions = [], scope }) => {
  const getInitials = () => {
    const init = record.title || 'l';
    return init.toString().substring(0, 2).toUpperCase();
  };

  const filteredActions = actions.filter((action: TableAction<ILearningPlanIndex>) => {
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-foreground text-lg truncate">{record.title || 'Unknown LearningPlan'}</h3>
              <p className="text-sm text-muted-foreground flex items-center mt-1">
                <Hash className="w-3 h-3 mr-1 flex-shrink-0" />
                <span className="truncate">{record.learningPlanId || 'N/A'}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        {filteredActions.length > 0 && (
          <div className="mt-4 pt-4 border-t border-border/50">
            <div className="flex gap-2">
              {filteredActions.map((action: TableAction<ILearningPlanIndex>, index: number) => (
                <Button
