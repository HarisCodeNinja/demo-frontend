...
interface LeaveApplicationCardProps {
  ...
  actions?: TableAction<ILeaveApplicationIndex>[];
  scope: string[];
}

export const LeaveApplicationCard: React.FC<LeaveApplicationCardProps> = ({ record, actions = [], scope }) => {
  const getInitials = () => {
    const init = record.leaveApplicationId || 'l';
    return init.toString().substring(0, 2).toUpperCase();
  };

  const filteredActions = actions.filter((action: TableAction<ILeaveApplicationIndex>) => {
